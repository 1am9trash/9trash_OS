...
run: build
	qemu-system-aarch64 -M raspi3b -kernel $(DES_DIR)/kernel8.img -serial null -serial stdio

debug: build
	qemu-system-aarch64 -M raspi3b -kernel $(DES_DIR)/kernel8.img -serial null -serial stdio -s -S
